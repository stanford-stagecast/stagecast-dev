...
def calculate_similarity_time_speedup(notes,source_id,currTime,timestamp_max_before_source=5000,zero_penalty=1,length_incentive=500000,max_offset=600,min_dist_const=400,skip=100,prev_matches=None,disp=False):
    """ Function that calls musical similarity on targets generated for a source_id.
        Target snips start at every 100 ms, and has same time length as source.
    ...
    if prev_matches is None or len(prev_matches)==0:
#         print("Nay!",end=" ")
#         print("\b"*42,end="")
#         print("No prev_matches: Searching without speedup",end="")
        return calculate_similarity_time(notes,source_id,currTime,timestamp_max_before_source,zero_penalty,length_incentive,max_offset,min_dist_const,skip,disp)
#     else:
#         print(len(prev_matches))
    
    num_matches = 5
    thresh = 0.7
    prev_matches_df = pd.DataFrame(data = prev_matches, columns=['source_timestamp', 'target_timestamp','score','source_id_start','source_id_end','target_id_start','target_id_end','match_len','match_time'])
    prev_matches_df = prev_matches_df.loc[prev_matches_df['score']>thresh].sort_values(by=['score'])
    prev_matches_best = np.array(prev_matches_df.iloc[-num_matches:],dtype=float)
    ...
    for match in prev_matches_best:
        target_start = int(match[1])+currTime-match[0]-100
        last_id_end = int(match[6])
        while target_start < match[1]+currTime-match[0]+101:
            target_end = target_start - length_ms # pick target_end by time length of course snip
...
#         print("Nay!",end=" ")
#         print("\b"*42,end="")
#         print("No new matches: Searching without speedup",end="")
        return calculate_similarity_time(notes,source_id,currTime,timestamp_max_before_source,zero_penalty,length_incentive,max_offset,min_dist_const,skip,disp)
    # else:
#         print("Yay!",end=" ")
    return matches
...
def find_matches_at_timestamp(i,notes,minNotes,minTime,maxNotes,maxTime,thresh,timestamp_max_before_source=5000,zero_penalty=1,length_incentive=500000,max_offset=600,min_dist_const=400,prev_matches=None,disp=False):
...
def get_source_notes_rev(notes, start_time, start_index, min_notes, max_notes, max_time):
    """ Function that ...
    ...
    Args:
        ...
        min_notes: min number of notes for a valid sequence, integer
        max_notes: max note length for a snippet, integer
        max_time: max time length for a valid sequence, integer (ms)\
    ...
    if start_index[0]<min_notes:
        return np.array([None,None])
    
    end_index = np.arange(start_index-min_notes, start_index - max_notes - 1, -1)
    ids = (start_time - notes[end_index,0] <= max_time) * end_index
    ids = ids[np.nonzero(ids)]
    if ids.any():
        return np.array([start_index[0],ids[-1]])
