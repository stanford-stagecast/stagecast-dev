...
def two_way_similarity(tf1,tf2,zero_penalty=1,length_incentive=500000,max_offset=600,min_dist_const=400,disp=False):
    a1,b1,c1,d1,score1 = musical_similarity(tf1,tf2,zero_penalty=zero_penalty,length_incentive=length_incentive,max_offset=max_offset,min_dist_const=min_dist_const,disp=disp)
    b2,a2,d2,c2,score2 = musical_similarity(tf2,tf1,zero_penalty=zero_penalty,length_incentive=length_incentive,max_offset=max_offset,min_dist_const=min_dist_const,disp=disp)
    if score1>score2:
...
#     pdb.set_trace()
    ...
    scores, mean_offset1, mean_offset2 = note_similarity_vect2_mean(sequence1,sequence2,time_ratio*(np.arange(sequence2.shape[0],0,-1)),max_offset=max_offset,min_dist_const=min_dist_const)
